...
function debounce(func: any, wait: number) {
  ...
  return function (this: any, ...args: any[]) {
    ...
    timeout = setTimeout(() => (func as any).apply(this, args), wait);
  };
}
...
function createTextTexture(gl: any, text: string, font = 'bold 30px sans-serif', color = '#DAA520') {
...
class Title {
  ...
  constructor({ gl, plane, renderer, text, textColor = '#ffffff', font = 'bold 28px sans-serif' }: any) {
    autoBind(this);
    this.gl = gl;
    this.plane = plane;
...
class Media {
    ...
    this.program = new Program(this.gl, {
      ...
      vertex: `
        ...
        void main() {
          vUv = uv;
          vec3 p = position;
          p.z = (sin(p.x * 3.0 + uTime) * 1.2 + cos(p.y * 1.5 + uTime) * 1.2) * (0.05 + uSpeed * 0.3);
  ...
  onResize({ screen, viewport }: any = {}) {
    ...
    // Slightly enlarged card dimensions as requested
...
class App {
  ...
  createMedias(items: any[], bend = 1, textColor: string, borderRadius: number, font: string) {
    const defaultItems = [
      ...
      { image: '/assets/villas/willow-peak/main.webp', text: 'Willow Peak (Lonavala)', link: '/villa/willow-peak' },
      { image: '/assets/villas/terra-cotta-villa/IMG-20260901-WA0061.jpg', text: 'Terra Cotta Villa (Mahabaleshwar)', link: '/villa/terra-cotta-villa' }
    ];
  ...
  onTouchMove(e: any) {
    ...
    const diff = Math.abs(this.start - x);
  ...
  onTouchUp(e: any) {
    ...
    if (this.dragDistance < 5 && this.onItemClick) {
      // Find closest item in center
      const currentScroll = this.scroll.target;
      if (this.medias && this.medias.length > 0) {
        const width = this.medias[0].width;
        let index = Math.round(currentScroll / width) % (this.mediasImages.length / 3);
        if (index < 0) index += this.mediasImages.length / 3;
        const clickedItem = this.mediasImages[Math.floor(index)];
...
export interface CircularGalleryProps {
  items?: Array<{ image: string; text: string; link?: string; desc?: string; price?: string }>;
  bend?: number;
  textColor?: string;
  borderRadius?: number;
  font?: string;
...
export default function CircularGallery({
  ...
  return (
    <div
      ...
      tabIndex={0}
      role="region"
      aria-label="Circular image gallery. Drag or scroll to explore stays."
    />
  );
}
